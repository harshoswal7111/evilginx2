...
func (t *Terminal) handleProxy(args []string) error {
	...
	if pn == 0 {
		...
		case "enable":
			...
			log.Important("you need to restart the application for the changes to take effect!")
			return nil
		case "disable":
...
func (t *Terminal) createHelp() {
	...
	h.AddSubCommand("phishlets", []string{"hostname"}, "hostname <phishlet> <hostname>", "set hostname for given phishlet (e.g. this.is.not.a.phishing.site.example.com)")
